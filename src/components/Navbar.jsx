    ...
    decoding="async" /></Link>
        ...
        <ul className="hidden md:flex gap-6 text-sm font-medium tracking-[1.69px] text-white items-center">
          ...
          >
            <NavLink to="#"
              className={({ isActive }) =>
                `nav-desktop-link px-3 py-1.5 rounded-[5px] uppercase inline-flex items-center gap-1.5 !outline-none ${
                  isActive ? "active-link  text-white" : "hover:text-green-400"}`}>
              Company
      ...
      >
        ...
        <div className="mobile-links-wrap px-4 py-5">
          ...
          {/* Home */}
          <NavLink key={`home-${flipKey}`} to="/" end onClick={() => setOpen(false)}
            className={({ isActive }) =>
              `mobile-flip-link flip-open-1 ${isActive ? "active-mobile" : ""}`}>
            Home
          </NavLink>
            ...
            <div className={`mobile-sub-container ${mobSub ? "open" : ""}`}>
              {ABOUT_SUB.map(({ to, label }) => (
                <NavLink key={to} to={to}
                  ...
                  className={({ isActive }) =>
                    `mobile-sub-link ${isActive ? "active-mobile-sub" : ""}`}>
          ...
          {/* Branches */}
          <NavLink key={`branches-${flipKey}`} to="/branches" onClick={() => setOpen(false)}
            className={({ isActive }) =>
              `mobile-flip-link flip-open-3 ${isActive ? "active-mobile" : ""}`}>
            Branches
          </NavLink>

          {/* Contact */}
          <NavLink key={`contact-${flipKey}`} to="/contact" onClick={() => setOpen(false)}
            className={({ isActive }) =>
              `mobile-flip-link flip-open-4 ${isActive ? "active-mobile" : ""}`}>
            Contact Us
          </NavLink>
